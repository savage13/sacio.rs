...
fn sac_data_read(file: &mut File, h: &mut sac) {
    ...
    h.y = RefCell::new(vec![0.0; npts]);
    let mut y = h.y.borrow_mut();
    let buffer = vec_to_u8(&mut y, hsize);
    file.read_exact(buffer).unwrap();
}

fn sac_data_write(file: &mut File, s: &mut sac, npts: usize) {
    let hsize : usize = npts * mem::size_of::<f32>();
    let mut y = s.y.borrow_mut();
    let hslice = vec_to_u8(&mut y, hsize);
...
impl sac {
    ...
    pub fn max_amp(&self) -> f32 {
        let mut vmax = self.y.borrow()[0];
        for v in self.y.borrow().iter() { if *v > vmax { vmax = *v; } }
        vmax
    }
    pub fn min_amp(&self) -> f32 {
        let mut vmin = self.y.borrow()[0];
        for v in self.y.borrow().iter() { if *v < vmin { vmin = *v; } }
        vmin
    }
    pub fn mean_amp(&self) -> f32 {
        let mut vmean : f64 = 0.0;
        for v in self.y.borrow().iter() { vmean = vmean + (*v as f64); }
    ...
    pub fn exp(&mut self) {
        for v in self.y.borrow_mut().iter_mut() {
            *v = v.exp();
        }
        self.extrema_amp();
    }
    pub fn is_finite(&self) -> bool {
        self.y.borrow().iter().all(|x| x.is_finite() == true)
    }
    pub fn integrate(&mut self) {
        ...
        // Trapezodial Integration
        {
            ...
            for i in 0 .. npts {
                let pt = hstep * (y[i] + y[i+1]);
                yt = yt + pt;
                y[i] = yt;
            }
            y.pop(); // Remove Last Point 
        }
...
/* F32 op SAC */
impl ops::Add<sac> for f32 {
    type Output = sac;
    fn add(self, x: sac) -> sac {
        return x + self
    }
}
...
impl ops::Add<sac> for sac {
    type Output = sac;
    fn add(self, x: sac) -> sac {
        let mut s = self.clone();
        {
            let mut y = s.y.borrow_mut();
            let v = x.y.borrow_mut();
            let npts = s.npts as usize;
            for i in 0 .. npts {
                y[i] = y[i] + v[i];
            }
...
/* SAC op F32 */
impl ops::Add<f32> for sac {
    type Output = sac;
    fn add(self, x: f32) -> sac {
        let mut s = self.clone();
        for v in s.y.borrow_mut().iter_mut() {
            *v = *v + x;
        }
        s.extrema_amp();
        s
...
impl ops::Sub<f32> for sac {
    type Output = sac;
    fn sub(self, x: f32) -> sac {
        return self + (-x);
    }
}
impl ops::Mul<f32> for sac {
    type Output = sac;
    fn mul(self, x: f32) -> sac {
        let mut s = self.clone();
        for v in s.y.borrow_mut().iter_mut() {
            *v = *v * x;
        }
...
impl ops::Div<f32> for sac {
    type Output = sac;
    fn div(self, x: f32) -> sac {
        let mut s = self.clone();
        for v in s.y.borrow_mut().iter_mut() {
            *v = *v / x;
        }
...
mod tests {
    ...
    fn create_time() {
        ...
        assert_eq!(s.nvhdr,   6);
        assert_eq!(*s.y.borrow(),     vec![0.,-1.,2.0]);
    }
    #[test]
    fn test_exp() {
        let mut s = sac::from_amp(vec![1.,2.,3.], 0.0, 1.0);
        s.file = String::from("test_exp");
        s.exp();
        let mut v = vec![1.0, 2.0, 3.0];
        for i in 0 .. v.len() {
            v[i] = (v[i] as f32).exp()
        }
        assert_eq!(*s.y.borrow(), v);
    }

    #[test]
    fn test_finite() {
        let mut s = sac::from_amp(vec![1.,2.,3.], 0.0, 1.0);
        s.file = String::from("test_finite_ok");
        assert_eq!(s.is_finite(), true);
        let mut s = sac::from_amp(vec![1.,2.,1./0.], 0.0, 1.0);
        s.file = String::from("test_finite_nan");
        assert_eq!(s.is_finite(), false);
        let mut s = sac::from_amp(vec![1.,2.,-1./0.], 0.0, 1.0);
        s.file = String::from("test_finite_neg_nan");
        assert_eq!(s.is_finite(), false);
    }
    ...
    fn test_integrate() {
        ...
        s.integrate();
        assert_eq!(*s.y.borrow(), &[1.5, 4.]);
    }
    #[test]
    fn test_differentiate() {
        let mut s = sac::from_amp(vec![1.,2.,3.], 0.0, 1.0);
        s.file = String::from("test_differentiate");
        s.differentiate();
        assert_eq!(*s.y.borrow(), &[1.0, 1.0]);
    }

    #[test]
    fn test_add() {
        let mut s = sac::from_amp(vec![1.,2.,3.], 0.0, 1.0);
        s.file = String::from("test_add_s #1");
        assert_eq!(*s.y.borrow(), &[1.,2.,3.]);
        let mut s1 = s + 1.0;
        s1.file = String::from("test_add_s+1");
        assert_eq!(*s1.y.borrow(), &[2.,3.,4.]);
    ...
    fn test_div() {
        ...
        assert_eq!(*s1.y.borrow(), &[0.5,1.,1.5]);
    }
    #[test]
    fn test_clone() {
        let mut s = sac::from_amp(vec![1.,2.,3.], 0.0, 1.0);
    ...
    fn test_assign_op() {
        ...
        s.file = String::from("test_clone s");
        s += 1.0;
        assert_eq!(*s.y.borrow(), &[2.,3.,4.]);
        s *= 2.0;
        assert_eq!(*s.y.borrow(), &[4.,6.,8.]);
        s -= 3.0;
        assert_eq!(*s.y.borrow(), &[1.,3.,5.]);
        s /= 2.0;
        assert_eq!(*s.y.borrow(), &[0.5,1.5,2.5]);
    }
